...
import { Terminal, Copy, Info, BarChart3, Clock, UserCheck, Layout, FileText, ChevronRight, Zap } from 'lucide-react';
      ...
      <div className="px-10 py-8 bg-white border-b border-slate-50">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-8">
            <div className={`p-5 rounded-[1.5rem] bg-slate-50 border border-slate-100 text-blue-600 shadow-inner group-hover:scale-105 transition-transform`}>
            ...
            <div>
              <div className="flex items-center gap-4 mb-2">
                ...
                <div className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-extrabold uppercase tracking-widest ${
                  node.status === NodeStatus.COMPLETED ? 'bg-emerald-50 text-emerald-600' :
                  node.status === NodeStatus.PROCESSING ? 'bg-blue-50 text-blue-600 animate-pulse' :
                  'bg-slate-50 text-slate-400'
                }`}>
                  <span className={`w-1.5 h-1.5 rounded-full ${
                    node.status === NodeStatus.COMPLETED ? 'bg-emerald-500' :
                    node.status === NodeStatus.PROCESSING ? 'bg-blue-500' : 'bg-slate-300'
                  }`}></span>
                  {node.status}
                </div>
              </div>
              <div className="flex items-center gap-6 text-[11px] font-bold text-slate-400 uppercase tracking-wider">
                <span className="flex items-center gap-2">
                  ...
                  Operator: <span className="text-slate-600">{node.agent}</span>
                </span>
                {node.details.timeTaken && (
                  <span className="flex items-center gap-2">
                    <Clock size={14} />
                    Execution: <span className="text-slate-600">{node.details.timeTaken}</span>
                  </span>
                )}
              </div>
            </div>
      ...
      <div className="flex-1 overflow-y-auto p-10 space-y-10 bg-white custom-scrollbar">
        ...
        {node.status === NodeStatus.PENDING ? (
          <div className="h-full flex flex-col items-center justify-center text-slate-300 space-y-6">
            ...
            <p className="font-bold text-sm tracking-[0.3em] uppercase opacity-50">Waiting for Signal</p>
          </div>
        ) : (
          <>
              ...
              {node.details.strategy && (
                <div className="p-6 rounded-3xl bg-blue-50/30 border border-blue-100/50 hover:shadow-lg transition-shadow">
                  <div className="flex items-center gap-3 text-blue-600 mb-3">
                    ...
                    <span className="text-[10px] font-extrabold uppercase tracking-[0.2em]">Strategy Insight</span>
                  </div>
              ...
              {node.details.readabilityScore !== undefined && (
                <div className="p-6 rounded-3xl bg-emerald-50/30 border border-emerald-100/50 hover:shadow-lg transition-shadow">
                  <div className="flex items-center gap-3 text-emerald-600 mb-3">
                    ...
                    <span className="text-[10px] font-extrabold uppercase tracking-[0.2em]">Quality Score</span>
                  </div>
            ...
            {node.status === NodeStatus.PROCESSING && (
              <div className="p-8 rounded-[2rem] bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-100 flex items-center gap-8 animate-in fade-in duration-500">
                <div className="flex-shrink-0 w-20 h-20 rounded-full bg-white flex items-center justify-center shadow-lg ring-8 ring-blue-500/5">
                  <span className="text-lg font-black text-blue-600">{node.details.progress}%</span>
                </div>
                <div className="flex-1 space-y-3">
                  <h4 className="text-[11px] font-black text-blue-600 uppercase tracking-widest">Active Execution Pipeline</h4>
                  <div className="flex items-center gap-2 text-slate-600 font-bold text-base">
                    <ChevronRight size={18} className="text-blue-500" />
                    {node.details.currentAction || "Initializing sub-tasks..."}
                  </div>
                  {node.details.eta && <p className="text-slate-400 text-xs font-medium italic">Estimated finish: {node.details.eta}</p>}
                </div>
              </div>
            )}
            ...
            {node.details.goldenQuotes && node.details.goldenQuotes.length > 0 && (
              <div className="space-y-4">
                <label className="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em]">Extracted Keyphrases</label>

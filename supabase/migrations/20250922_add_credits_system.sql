-- Add credits system for video generation billing
-- Convert existing balance (USD) to credits (10000 credits = $10)

-- Add credits column to users table
ALTER TABLE public.users 
ADD COLUMN IF NOT EXISTS credits INTEGER DEFAULT 50000; -- 50000 credits = $50
...
CREATE TABLE IF NOT EXISTS public.credits_transactions (
    ...
    description TEXT,
    session_id TEXT, -- for tracking video generation sessions
    video_url TEXT, -- final video URL when video generation is complete
    metadata JSONB DEFAULT '{}', -- store additional data like duration, model used, etc.
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);

-- Create video_generations table for tracking video creation status
CREATE TABLE IF NOT EXISTS public.video_generations (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID NOT NULL REFERENCES public.users(id) ON DELETE CASCADE,
    session_id TEXT NOT NULL UNIQUE,
    duration INTEGER NOT NULL, -- video duration in seconds
    credits_cost INTEGER NOT NULL, -- credits to be charged
    status TEXT NOT NULL DEFAULT 'pending' CHECK (status IN ('pending', 'processing', 'completed', 'failed', 'cancelled')),
    video_url TEXT, -- final video URL
    n8n_workflow_data JSONB DEFAULT '{}', -- store N8N workflow request/response
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);

-- Migrate existing USD balance to credits (50 USD = 50000 credits)
UPDATE public.users 
...
-- Service can insert video generations and transactions
CREATE POLICY "Service can manage video generations" ON public.video_generations 
FOR ALL USING (true);

CREATE POLICY "Service can manage credit transactions" ON public.credits_transactions 
FOR ALL USING (true);
...
-- Function to deduct credits from user and create transaction record
...
-- Function to add credits to user (for top-ups)

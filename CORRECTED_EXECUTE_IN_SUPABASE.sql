-- =====================================================
-- Video Tracking System Migration (FINAL CORRECTED)
-- Auto-video directly deducts credits (no USD conversion)
-- Deep Copywriting uses USD->credits conversion (unchanged)
-- COPY AND PASTE THIS INTO SUPABASE SQL EDITOR
-- =====================================================

-- Step 1: Create video_generations table (using credits directly)
CREATE TABLE IF NOT EXISTS public.video_generations (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID NOT NULL REFERENCES public.users(id) ON DELETE CASCADE,
    session_id TEXT NOT NULL UNIQUE,
    duration INTEGER NOT NULL,
    cost_credits INTEGER NOT NULL, -- Store credits directly, not USD
...
DECLARE
    ...
    new_credits INTEGER;
BEGIN
    -- Get current credits (stored in balance field)
    SELECT balance INTO current_credits FROM public.users WHERE id = user_uuid;
    ...
    -- Update user balance (same as existing Deep Copywriting system)
    ...
    -- Create billing record for video
...
BEGIN
    ...
    -- If video generation failed, refund the credits
...
SELECT 'Video tracking migration completed successfully!' AS status;
